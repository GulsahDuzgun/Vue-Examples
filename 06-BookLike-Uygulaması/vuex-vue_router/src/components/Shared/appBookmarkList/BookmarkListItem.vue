    ...
    <div  class="bg-white flex flex-col gap-x-3 rounded-md shadow-sm">
        <div class="p-3">
          ...
          <div class="text-xs text-gray-400 mt-2 flex justify-between">
            <a href="#" class="hover:text-black">{{getUserName}}</a>
            <span>14 Mart</span>
          </div>
        </div>
        <div class="bg-red-200 p-1 text-red-900 text-center text-sm">{{getCategoryName}}</div>
      </div>
</template>
<script>
...
export default{
  ...
  computed:{
    ...mapGetters(["_geCurrentUserId","_getCurrentUser","_getUserBookmarks","_getUserLikes"]),
    getCategoryName(){
  ...
  methods:{
    likeItem(){
      ...
      // console.log(this._getUserLikes)
      ...
      this.$appAxios.patch(`/users/${this._getCurrentUser.id}`,{likes}).then(likes_response =>
       {
